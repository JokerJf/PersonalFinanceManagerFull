...
const BLOCK_DURATION = 30 * 60 * 1000; // 15 minutes block
const ALLOW_DURATION = 30 * 60 * 1000; // 15 hour allow after block
...
const ddosLimiter = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  if (!ipData || ipData.resetTime < now - WINDOW_MS) {
    ipData = { count: 0, resetTime: now, blocked: false, blockUntil: 0 };
  }
